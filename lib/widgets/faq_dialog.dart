...
class FAQDialog extends StatelessWidget {
  ...
  static const _sections = <_FAQSection>[
    _FAQSection(
      ...
      title: 'Главная идея',
      subtitle: 'Навык → этап → квест → минимальный шаг → XP → рост.',
      bullets: [
        'Навык — направление развития: Python, спорт, проект, привычка.',
        'Этап — ступень мастерства внутри навыка: основа, практика, проект.',
        'Квест — конкретное действие, а минимальный шаг помогает начать без давления.',
      ],
    ),
    _FAQSection(
      ...
      bullets: [
        'Сначала поднимаются повторяющиеся квесты под риском сброса.',
        'Затем приложение ищет квесты с минимальным шагом и активным этапом.',
        'Ручной фокус, XP и дата используются только как tie-breakers, а не как главный смысл.',
      ],
    ),
    _FAQSection(
      icon: Icons.flag,
      ...
      bullets: [
        'Квест отвечает на вопрос “что сделать?”, минимальный шаг — “с чего начать?”.',
        'Минимальный шаг даёт часть XP и помечает квест как начатый.',
        'Полное выполнение закрывает квест и фиксирует рост навыка.',
      ],
    ),
    _FAQSection(
      icon: Icons.bolt,
      ...
      bullets: [
        ...
        'Ранги временно заморожены: основной прогресс сейчас строится вокруг уровней.',
      ],
    ),
    _FAQSection(
      icon: Icons.redeem,
      ...
      subtitle: 'Это отклик после действия, а не отдельная экономика.',
      bullets: [
        'Сундуки выдаются за сильный день, серию или событие сопротивления.',
        'Открытый сундук создаёт пассивный эффект: бонус XP применится сам.',
        'Если отменить квест, трофей откатывается, когда условие больше не выполнено.',
      ],
    ),
    _FAQSection(
      icon: Icons.shield,
      ...
      title: 'Сопротивление',
      subtitle:
          'Это образ препятствия, которое слабеет от реального прогресса.',
      bullets: [
        'Сопротивление слабеет от квестов, минимальных шагов и освоения этапов.',
        'Это событие прогресса, а не квест, который нужно обслуживать каждый день.',
        'Победа может дать сундук, но главный смысл — показать, что путь стал легче.',
      ],
    ),
    _FAQSection(
      ...
      subtitle: 'Большой навык можно разложить на понятные этапы.',
      bullets: [
        'Этап — ступень навыка: основы, практика, проект или отдельная техника.',
        'Квесты можно связать с этапом, чтобы видеть, что именно двигает мастерство.',
        'Освоение этапа даёт XP и становится событием в истории роста.',
      ],
    ),
    _FAQSection(
  ...
  static const _quickStart = <String>[
    'Создай навык: первый этап и первый квест появятся сразу.',
    'Заполни минимальный шаг — это вход без давления.',
    'Каждый день начинай с “Действовать”, а не с настройки системы.',
    'После выполнения смотри рост в “Статистике” и путь в “Карте”.',
  ];
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Коротко о пути: навык, этап, квест, минимум и рост.',
                              style: TextStyle(
                                color: sub,
                                fontSize: 12.5,
                                height: 1.25,
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              'Лучший режим использования: не планировать идеально, а каждый день закрывать хотя бы один маленький квест или минимальный шаг. Система сильнее всего работает, когда помогает начать.',
