...
class _TasksPanelState extends State<TasksPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    if (s.selectedSkillId != _lastSkillId) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted && _checklistExpanded) setState(() => _checklistExpanded = false);
      });
    }
    ...
    if (skill == null) {
      return Container(
        decoration: BoxDecoration(
          color: sfc, borderRadius: BorderRadius.circular(14), border: Border.all(color: bdr)),
        child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.arrow_back, color: sub, size: 30),
          const SizedBox(height: 12),
          Text('Выберите навык', style: TextStyle(color: sub, fontSize: 16, fontWeight: FontWeight.w500)),
          const SizedBox(height: 4),
          Text('Задачи откроются здесь', style: TextStyle(color: sub.withAlpha(160), fontSize: 13)),
        ])),
      );
    }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          child: Row(
            ...
            children: [
              ...
              Container(
                width: 32, height: 32,
                decoration: BoxDecoration(
                  color: skill.color.withAlpha(30), borderRadius: BorderRadius.circular(8)),
        ...
        Padding(
          ...
          child: Row(children: [
            Expanded(child: XPBar(progress: skill.progress, color: skill.color, height: 6)),
            const SizedBox(width: 8),
            Text('${skill.xp} / ${skill.xpNeeded} XP  •  Ур.${skill.level}',
                            ...
                            child: Row(children: [
                              AnimatedContainer(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  border: Border.all(
                                      color: isDone ? skill.color : sub, width: 1.5),
        ...
        Expanded(
          child: allTasks.isEmpty
              ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Text('Нет задач',
                      style: TextStyle(color: sub, fontSize: 15, fontWeight: FontWeight.w500)),
                  const SizedBox(height: 4),
                  Text('Нажмите «+ Задача»',
                      style: TextStyle(color: sub.withAlpha(150), fontSize: 12)),
                ]))
              : ListView(
                  padding: const EdgeInsets.symmetric(vertical: 8),
                  children: [
                    ...active.map((t) => TaskTile(
                      task: t, isDark: isDark, skillColor: skill.color,
                    ...
                    if (done.isNotEmpty) ...[
                      Padding(
                        ...
                        child: Row(children: [
                          ...
                          Text('Выполнено сегодня (${done.length})',
  ...
  void _addTask(BuildContext ctx, Skill skill) {
    ...
    showDialog(context: ctx, builder: (_) => AddTaskDialog(
      isDark: s.isDark, skillColor: skill.color,
      onSave: (title, xp, type, freq, customDays) => s.addTask(
        Task(id: uid(), title: title, skillId: skill.id, xpReward: xp, type: type,
             repeatFrequency: freq, repeatCustomDays: customDays),
      ),
    ));
  }
  ...
  void _editTask(BuildContext ctx, Skill skill, Task task) {
    final s = AppStateProvider.of(ctx);
    showDialog(context: ctx, builder: (_) => AddTaskDialog(
      isDark: s.isDark, skillColor: skill.color, existing: task,
      onSave: (title, xp, type, freq, customDays) {
        task.title = title; task.xpReward = xp; task.type = type;
        task.repeatFrequency = freq; task.repeatCustomDays = customDays;
        s.refresh();
      },
    ));
  }
...
class _ChecklistBtn extends StatefulWidget {
  ...
  const _ChecklistBtn({
    required this.done, required this.total, required this.expanded,
    required this.color, required this.onTap,
  });
  @override State<_ChecklistBtn> createState() => _ChecklistBtnState();
}
...
class _ChecklistBtnState extends State<_ChecklistBtn> {
  ...
  @override
  Widget build(BuildContext context) {
    final bg = widget.expanded ? darken(widget.color) : widget.color;
    return GestureDetector(
      ...
      child: AnimatedScale(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: _p ? darken(bg) : bg,
            borderRadius: BorderRadius.circular(8),
          ),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(
              widget.expanded ? Icons.check_box : Icons.check_box_outline_blank,
...
class TaskTile extends StatefulWidget {
  ...
  const TaskTile({
    super.key,
    required this.task, required this.isDark, required this.skillColor,
    required this.onToggle, required this.onUncomplete, required this.onDelete, required this.onEdit,
  });
  @override State<TaskTile> createState() => _TaskTileState();
}
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: _h ? (isDark ? const Color(0xFF1E1E2A) : const Color(0xFFECECF8)) : tileBg,
          borderRadius: BorderRadius.circular(10),
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
            PressFeedback(
              ...
              onTap: () {
                if (t.isDone) {
                  ...
                  final box = _cbKey.currentContext?.findRenderObject() as RenderBox?;
                  widget.onToggle(box?.localToGlobal(Offset.zero) ?? Offset.zero);
                }
              },
              child: AnimatedContainer(
                key: _cbKey,
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: t.isDone ? widget.skillColor : sub, width: 2),
                  color: t.isDone ? widget.skillColor : Colors.transparent,
                ),
                child: t.isDone ? const Icon(Icons.check, size: 13, color: Colors.white) : null,
              ),
            ),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Wrap(spacing: 6, runSpacing: 4, children: [
                TaskBadge(label: typeLabel[t.type]!, color: typeColor[t.type]!),
                TaskBadge(
                  icon: Icons.auto_awesome,
                  label: t.isDone ? '-${t.earnedXP} XP' : '+${t.xpReward * mult} XP',
                  color: t.isDone ? sub : const Color(0xFF4A9EFF),
                ),
                if (t.showStreakBadge)
                  TaskBadge(icon: Icons.local_fire_department, label: '×$mult',
                      color: const Color(0xFFFF9500)),
                if (t.streak > 0 && t.type == TaskType.repeating)
                  Text('${t.streak} д.', style: TextStyle(color: sub, fontSize: 11)),
                if (t.type == TaskType.repeating) ...[
                  TaskBadge(icon: Icons.repeat, label: freqLabel[t.repeatFrequency]!,
                      color: const Color(0xFF4A9EFF)),
                  if (t.isDone && t.nextResetAt != null)
                    Text(formatResetLabel(t.nextResetAt), style: TextStyle(color: sub, fontSize: 11)),
                ],
              ]),
            ])),
            AnimatedContainer(
              ...
              child: SingleChildScrollView(
                ...
                child: SizedBox(width: 44, child: Row(mainAxisSize: MainAxisSize.min, children: [
                  MiniBtn(icon: Icons.edit, color: sub, onTap: widget.onEdit),
                  MiniBtn(icon: Icons.delete_outline,
                      color: const Color(0xFFFF3B30), onTap: widget.onDelete),
                ])),
              ),
            ),
          ]),
