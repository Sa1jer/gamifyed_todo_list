...
/// Тень под белым текстом, лежащим на цветной подложке.
///
/// Цвет подложки выбирает пользователь, и на светлых — жёлтом, салатовом,
/// голубом — белая надпись теряется: контраст там около 1.5:1 при пороге 4.5.
/// Владелец выбрал белый везде ради единообразия кнопки; тень возвращает часть
/// читаемости, не трогая цвет.
///
/// Два слоя. Нижний — широкий и ровно по центру: он приглушает подложку под
/// всей надписью разом, и именно он делает основную работу на светлом фоне.
/// Верхний — узкий и смещённый на пиксель вниз: он даёт кромку, за которую
/// цепляется глаз, и читается как тень, а не как ореол.
///
/// Одного центрального слоя мало — без кромки буквы выглядят размытыми; одного
/// смещённого мало — он не закрывает подложку над буквой. Порядок важен:
/// широкий рисуется первым, узкий поверх.
///
/// На тёмной подложке оба невидимы и ничего не стоят.
const List<Shadow> onColorInkShadows = [
  Shadow(color: Color(0x4D000000), blurRadius: 9),
