...
- [x] Real-Hive fault-injection and CI storage regression coverage: legacy dual-read, corrupt current payload falling back to previous, corrupt box bytes, interrupted write into a closed box leaving the committed snapshot intact, and idempotent repeat saves — all against real Hive in `storage_service_reopen_test.dart`, run by CI in the normal suite. Note: Hive recovers a corrupted box by discarding its contents, so the snapshot layer — not the legacy boxes — is what protects the data.
...
- [x] Encryption policy for the user-data transfer file: **owner decision — no password, the file stays plain JSON**. One exported file carries every skill, quest, history entry and profile field in the clear; treated as a trusted-device transfer, not a backup format for untrusted channels.
- [ ] Consider legacy-box cleanup only after several releases of verified snapshot recovery. The export/restore precondition is now met by the profile data transfer. This now also covers the boxes left behind in the old `getApplicationDocumentsDirectory()` location by the 2026-09-04 move to `<app support>/storage`: the migration deliberately copies rather than moves, so the old files are the way back until the new location has proven itself.
...
- [ ] Validate native Windows/macOS pointer tracking. The code fixes landed (`e8685a6` centralized hover ownership; the transparent-black lerp that flashed grey on every hover is gone), so this is a native compositor check, not an open bug. Capture a platform repro only if hover still differs.
- [ ] Re-measure mobile theme switching before considering a reduced-motion fallback. The stutter had a found cause and it is fixed: the reveal animation used to start on the same frame that rebuilt and rasterized the whole tree in new colours and uploaded a full-screen 2x texture; two frames are now skipped under the closed circle and the image has its own repaint boundary. The `2x` cap still stands. No timings captured since.
...
- [x] Sustained-mutation storage stress test: 40 mutations with no pause collapse into under ten snapshot writes without losing the last one, and state survives a process-like reopen (`test/persistence/sustained_mutation_test.dart`).
- [x] `flushSaves()` now skips the write when `PersistenceStatus.isDirty` is false. The dirty-state-after-failed-save guarantee still holds and is covered directly: a failed write puts the flag back and the next flush retries. The background transition was never the cost the earlier note claimed — `pauseBackgroundWork()` already checked the same flag; the redundant writes were on the direct callers (export, retry, debug). A pending debounce is deliberately left running rather than cancelled, so a skipped flush cannot swallow queued work.
...
- [x] Remove the remaining artificial one-frame mobile transition delay, isolate stable RoadMap paint layers, stop the unopened epic chest from repainting forever, and add opt-in frame-timing diagnostics that report the actual display refresh rate plus build/raster percentiles.
...
- [x] One in-memory storage fake for tests (`test/helpers/in_memory_storage_service.dart`) instead of the two that had drifted apart, and it carries a real in-memory snapshot backend rather than a faked `supportsSnapshots`. This unblocked the coverage that was missing because a real Hive store inside `testWidgets` never finishes its I/O: the undo snackbar for a deleted quest, and the data transfer controls in both profile layouts and in desktop settings.
- [x] RoadMap stage selection was never broken; the tests were tapping the wrong thing. `node-button-<skill>-<node>` covers the whole 155x163 node box, and `tester.tap` aims at its centre, which lands in the gap between the badge and the label where nothing is painted — `deferToChild` ignores it. Tapping the orb surface rect works, and the delete-confirmation test that was skipped for this reason now exists and fails against code without the confirmation. Note for future RoadMap tests: tap `map-node-surface-…`, not the item box, and remember a second tap on a selected stage deselects it.
...
- [x] Desktop audit wave 2, the part that was still true: the progress badge moved out of the orb's stack into the column flow, so it no longer straddles the ring and a two-line label clears it; locked stages two tiers apart from the frontier now dim, so a path stops reading as a row of identical locks. Decoupled `_roadmapNodeOrbTopOffset` from the horizontal content height — the two were numerically equal by a tuned constant, so any change to the horizontal composition silently moved vertical insertion points.
- [x] Wave 2 leftovers, resolved by the owner. Auto-fit scale floor: stays absent, because a floor conflicts with the deliberate contract that a ten-stage chain fits on screen (`vertical RoadMap auto-fits a ten-stage chain`). Vertical axis direction: confirmed as intended — the skill sits at the top and stages climb toward it, so the audit's request to mirror the horizontal axis is declined. Connection colour keys off the target stage's status, which highlights where you are rather than where you have been.
- [x] One-sided vertical stage labels. Stages standing on the axis now all label right and read as a column; only branch stages still label outward, because a 196px label pointed inward would run over the neighbouring path 180px away. My earlier claim that one-sided labels collide at the 170px pitch was wrong — it described the branch case, not the straight chain.
...
- [x] Android, iOS and macOS moved off `com.example...` to `io.github.sa1jer.rpgtodo` (2026-09-04), verified against the built APK with `aapt2 dump packagename`. Done early on purpose: a new application ID installs as a different app, so every ID change costs testers their local data unless they export first.
- [x] Final identifier settled: `io.github.sa1jer.rpgtodo`, publication included. Rather than buy a domain for reverse-DNS, this uses the GitHub Pages convention where the `Sa1jer` account establishes the claim to `sa1jer.github.io`. Do not revisit — after publication the ID cannot change, and before it every change costs testers their local data.
...
- [x] Publishing the signed APK to GitHub Releases is `tool/release.sh`: it bumps the build number, keeps `kAppVersionLabel` in step with pubspec, builds, asks the artifact itself which certificate signed it, and publishes with `gh release create` under a tag carrying the build number. The keystore deliberately stays off CI — it cannot be rotated without every tester reinstalling, so a repository secret would be a permanent exposure to save one command.
- [x] The in-app update notice reads the latest release once per launch, compares build numbers rather than display names, and lights a row in the profile when the installed build is behind. Every failure path is silence. It stops at a copyable link: opening a browser needs a dependency the project does not have, and downloading the APK would need `REQUEST_INSTALL_PACKAGES`, which is sensitive and closes the door on store distribution.
- [x] `url_launcher` added so the update notice opens the release page instead of asking the reader to paste a link. It is published by the Flutter team and covers every platform this app targets. The clipboard remains the fallback when a browser will not open — without the link there is no way to update at all, so that path is a rescue, not an error message. Android needs a `<queries>` entry for `https` VIEW intents or the browser is invisible to the app from API 30 on.
- [ ] macOS is not published with releases yet: distributing outside the App Store needs a `Developer ID Application` certificate and notarisation, and only an `Apple Development` certificate exists. Without it Gatekeeper refuses to open the app normally. Needs a paid Apple Developer account, or documented right-click-Open instructions if shipped anyway.
- [ ] iOS cannot be distributed this way at all — the system will not install an app from a downloaded file. TestFlight or an ad-hoc profile naming each device, both requiring the paid account.
...
- [x] Release build number moved off `+1` (now `1.3.64+2`) and platform metadata verified against the built APK: `io.github.sa1jer.rpgtodo`, versionName `1.3.64`, minSdk 24, targetSdk 36, label `RPG To-Do List`. Every build handed to a tester needs a higher `versionCode` than the one it replaces, or Android refuses the update — see `docs/BETA_DISTRIBUTION.md`.
...
- `1.3.64`: anchored action-context reward feedback to the exact completion-control rectangle and a bounded local halo, replacing temporary coordinate adapters across desktop, mobile, and RoadMap task paths. XP, completion, storage, and reward rules are unchanged; native visual QA remains pending.
