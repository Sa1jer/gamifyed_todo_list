...
- [ ] Wave 2 leftovers are owner decisions, not defects. Auto-fit scale floor: a floor conflicts with the deliberate contract that a ten-stage chain fits on screen (`vertical RoadMap auto-fits a ten-stage chain`), so pick one — whole map but small, or readable but panned. Vertical axis direction: the audit called it reversed, but `MobileRoadMapAscentLayout` documents the climb as intent — foundation at the bottom, stages rising toward the skill. One-sided labels: sides alternate to keep 196px labels from colliding at a 170px pitch, and a test asserts it. Connection colour keys off the target stage's status, which highlights where you are rather than where you have been.
