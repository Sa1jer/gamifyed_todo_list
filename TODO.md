...
- [ ] **Publish the signed release APK to GitHub Releases at the end of each large batch or version commit** (owner request, 2026-09-04; design not started). Decide first where the build runs. Building in CI means putting the keystore and its password into repository secrets, which is a real exposure for a key that cannot be rotated without every tester reinstalling; building locally and uploading with `gh release create` keeps the key on one machine at the cost of a manual step. A middle path is a workflow that only uploads an artifact the owner attaches. Whichever wins, the release tag must carry the `versionCode`, because that is what the updater below compares.
- [ ] **In-app update notice fed by those releases** (owner request, 2026-09-04; design not started). The app reads the latest published release from the GitHub API, compares it against its own build number, and lights a notice in Profile when it is behind. Constraints worth settling before any code: the repository is public but the API is rate-limited for unauthenticated clients, so the check needs caching and must fail silently offline rather than nagging; the comparison should use `versionCode` rather than the display name, since only the former is ordered; and if the notice ever grows into downloading and installing the APK, Android requires `REQUEST_INSTALL_PACKAGES`, which is a sensitive permission and is refused for Play-distributed apps — acceptable for sideloaded betas, blocking for a store build. Start with a notice and a link, not a self-updater.
