...
- [ ] Decide whether `flushSaves()` should skip the write when nothing is dirty. The characterization test found it writes unconditionally, so every background transition costs a full snapshot write even with no changes. Touching this changes persistence semantics — the dirty-state-after-failed-save guarantee has to keep holding.
